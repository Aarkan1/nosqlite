...
const mutations = {
    ...
    deleteObject(state, id) {
        ...
        state.collections = {...state.collections}
        fetch(`/rest/coll/${state.activeColl}/${id}`, { method: 'DELETE' })
    }
}
...
const actions = {
    async getCollNames(store) {
        ...
        let collNames = await fetch('/rest/coll-names')
        collNames = await collNames.json();
        collNames = JSON.parse(collNames)
    ...
    async getCollection(store, coll) {
        let data = await fetch('/rest/coll/' + coll)
